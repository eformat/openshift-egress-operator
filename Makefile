...
REGISTRY ?= quay.io
REPOSITORY ?= $(REGISTRY)/eformat/openshift-egress-operator
...
LDFLAGS := "-X github.com/eformat/openshift-egress-operator/version.Version=$(VERSION) \
	-X github.com/eformat/openshift-egress-operator/version.Vcs=$(BUILD_COMMIT) \
	-X github.com/eformat/openshift-egress-operator/version.Timestamp=$(BUILD_TIMESTAMP) \
	-X github.com/eformat/openshift-egress-operator/version.Hostname=$(BUILD_HOSTNAME)"
...
manager-local: generate fmt vet
	go build -o build/_output/bin/openshift-egress-operator-local -gcflags="all=-N -l" -ldflags $(LDFLAGS) github.com/eformat/openshift-egress-operator/cmd/manager

# Build manager binary
manager: generate fmt vet
	go build -o build/_output/bin/openshift-egress-operator  -ldflags $(LDFLAGS) github.com/eformat/openshift-egress-operator/cmd/manager
